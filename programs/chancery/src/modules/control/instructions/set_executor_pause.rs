/// set_executor_pause
///
/// Sets or clears the PERMISSION_FLAG_PAUSED bit on an executor's PermissionRecord.
/// Settlement handlers check this flag before allowing delegated execution.
///
/// Accounts:
///   0  chancery_config        writable  PDA  (sequence_nonce bump for evidence)
///   1  event_authority        readable  PDA [b"event-authority"]
///   2  permission_record      writable  PDA [b"permission", executor, scope_kind, scope_key]
...
use crate::{
    error::ChanceryError,
    modules::{
        control::instructions::{is_executor_pause_eligible, PERMISSION_FLAG_PAUSED},
...
pub struct SetExecutorPauseArgs {
...
pub fn handle<'a>(accounts: &'a [AccountInfo<'a>], args_data: &[u8]) -> ProgramResult {
    ...
    let args = SetExecutorPauseArgs::try_from_slice(args_data)
    ...
    // Clearing requires ops or governance; setting allows emergency too.
    ...
    // Mixed executor/principal settlement roles may coexist on one pathway
    ...
    let (record_scope_kind, record_scope_key, previous_bits, final_bits) = {
        ...
        if !is_executor_pause_eligible(record.scope_kind, roles) {
