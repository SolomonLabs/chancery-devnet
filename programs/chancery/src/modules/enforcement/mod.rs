//! enforcement - LATER MODULE, EXCLUDED FROM THE MVP BUILD.
//!
...
//! ID `module::ENFORCEMENT` (0x0F); its wire IDs stay reserved and
//! append-only. No enforcement event structs are retained in
//! `evidence/emit/`; the MVP basic freeze/thaw surface lives in `control`.
//!
...
impl ChanceryModule for Module {
    const MODULE_ID: u8 = crate::constants::module::ENFORCEMENT;
