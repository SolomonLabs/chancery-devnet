/// redeem_trilateral - TrilateralAtomicSettlement, redeem path.
///
/// Three-party: principal_a burns issued tokens (signs), principal_b receives
/// asset (signs), executor coordinates (signs). Title passes directly
/// principal_a -> principal_b through the burn/transfer flow.
///
...
///   9  source_issued_token_account                writable  principal_a burns from here
///  10  reserve_asset_token_account                writable
///  11  destination_asset_token_account            writable  principal_b receives asset
///  12  asset_mint                                 readable
///  13  issued_token_mint                          writable
///  14  reserve_authority_pda                      readable
///  15  asset_token_program                        readable
///  16  issued_token_program                       readable
///  17  executor                                   signer
///  18  principal_a                                signer    (burns issued tokens)
///  19  principal_b                                signer    (receives asset - confirms)
...
const SOURCE_ISSUED_TOKEN_ACCOUNT:       usize = 9;
const RESERVE_ASSET_TOKEN_ACCOUNT:       usize = 10;
const DESTINATION_ASSET_TOKEN_ACCOUNT:   usize = 11;
const ASSET_MINT:                        usize = 12;
const ISSUED_TOKEN_MINT:                 usize = 13;
const RESERVE_AUTHORITY_PDA:             usize = 14;
...
pub struct RedeemTrilateralArgs {
    pub intent_id : [u8; 32],
...
pub fn handle<'a>(accounts: &'a [AccountInfo<'a>], args_data: &[u8]) -> ProgramResult {
    ...
    let args  = RedeemTrilateralArgs::try_from_slice(args_data)
    ...
    if !asset_pause_state_account_info.data_is_empty() {
        ...
        )?.assert_not_paused_for(pause_bit::REDEEM, clock.slot)?;
    }
    ...
    let asset_destination_owner    = intent.principal_b;
    ...
    // ── 7. Burn first ─────────────────────────────────────────────────────────
    // BurnChecked accepts either the token-account owner or an approved
    // delegate. Bind the source to principal A explicitly so a principal who
    // happens to hold allowance over a third party's account cannot use that
    // account to fund collateral released to principal B.
    ...
    // principal_a signs the burn of their own issued tokens.
    cpi_burn(
        issued_token_program_account_info,
        source_issued_token_account_info,
        issued_token_mint_account_info,
        principal_a_account_info,
        issued_token_amount
    )?;

    // ── 8. Dimension limits + rate/fee application (spec §5.8) ────────────────
    ...
    // ── 9. Transfer asset to principal_b ──────────────────────────────────────
    ...
    if let Some((prepared_window_a, prepared_window_b)) = counterparty_daily_prepared {
        prepared_window_a.record_outflow(&accounts[COUNTERPARTY_A_DAILY_USAGE_WINDOW], gross_asset_out)?;
        prepared_window_b.record_outflow(&accounts[COUNTERPARTY_B_DAILY_USAGE_WINDOW], gross_asset_out)?;
    }
