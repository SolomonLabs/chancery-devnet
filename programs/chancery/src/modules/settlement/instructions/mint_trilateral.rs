/// mint_trilateral - TrilateralAtomicSettlement, mint path.
///
/// Three-party atomic settlement (spec 04 §4.1, §4.9):
///   principal_a  deposits collateral (signs)
///   principal_b  receives issued tokens (signs - confirms receipt)
///   executor     coordinates submission (signs)
///
/// Title passes directly principal_a -> principal_b.
/// Executor has no custody at any point.
///
/// Key differences from mint_delegated:
///   - pathway_kind = TRILATERAL
///   - principal_b also signs (not just principal_a)
///   - issued token destination is principal_b's account (not principal_a's)
///   - both principal permission records checked
///   - evidence records all three participants
///
...
///   6  principal_a_permission_record      readable
///   7  principal_b_permission_record      readable
///   8  executor_permission_record         readable
///   9  source_asset_token_account         writable  principal_a's collateral account
///  10  reserve_asset_token_account        writable  program-owned reserve
///  11  destination_issued_token_account   writable  principal_b's issued-token account
///  12  asset_mint                         readable
///  13  issued_token_mint                  writable
///  14  mint_authority_pda                 readable
///  15  asset_token_program                readable
///  16  issued_token_program               readable
///  17  executor                           signer
///  18  principal_a                        signer    (deposits)
///  19  principal_b                        signer    (receives - confirms)
///  20  asset_pause_state                  readable  PDA [b"asset-pause", asset_mint]
///  21  issued_token_control               readable  PDA [b"issued-token-control"]
/// Optional (22-30): fee_policy, fee_recipient_token_account, limit_policy,
/// hourly_usage_window, daily_usage_window, weekly_usage_window,
/// monthly_usage_window, evidence_policy, settlement_policy.
/// Dimension-limit optionals (31-37): asset_limit_policy,
/// asset_daily_usage_window, counterparty_limit_policy,
/// counterparty_a_daily_usage_window, counterparty_b_daily_usage_window,
/// executor_limit_policy, executor_daily_usage_window.
...
const PRINCIPAL_A_PERMISSION_RECORD:     usize = 6;
const PRINCIPAL_B_PERMISSION_RECORD:     usize = 7;
const EXECUTOR_PERMISSION_RECORD:        usize = 8;
const SOURCE_ASSET_TOKEN_ACCOUNT:        usize = 9;
const RESERVE_ASSET_TOKEN_ACCOUNT:       usize = 10;
const DESTINATION_ISSUED_TOKEN_ACCOUNT:  usize = 11;
const ASSET_MINT:                        usize = 12;
const ISSUED_TOKEN_MINT:                 usize = 13;
const MINT_AUTHORITY_PDA:                usize = 14;
const ASSET_TOKEN_PROGRAM:               usize = 15;
const ISSUED_TOKEN_PROGRAM:              usize = 16;
const EXECUTOR:                          usize = 17;
const PRINCIPAL_A:                       usize = 18;
const PRINCIPAL_B:                       usize = 19;
const ASSET_PAUSE_STATE:                 usize = 20;
const ISSUED_TOKEN_CONTROL:              usize = 21;
const REQUIRED_ACCOUNT_COUNT:            usize = 22;

const FEE_POLICY:                        usize = 22;
const FEE_RECIPIENT_TOKEN_ACCOUNT:       usize = 23;
const LIMIT_POLICY:                      usize = 24;
const HOURLY_USAGE_WINDOW:               usize = 25;
const DAILY_USAGE_WINDOW:                usize = 26;
const WEEKLY_USAGE_WINDOW:               usize = 27;
const MONTHLY_USAGE_WINDOW:              usize = 28;
const EVIDENCE_POLICY:                   usize = 29;
const SETTLEMENT_POLICY:                 usize = 30;
const ASSET_LIMIT_POLICY:                usize = 31;
const ASSET_DAILY_USAGE_WINDOW:          usize = 32;
const COUNTERPARTY_LIMIT_POLICY:         usize = 33;
const COUNTERPARTY_A_DAILY_USAGE_WINDOW: usize = 34;
const COUNTERPARTY_B_DAILY_USAGE_WINDOW: usize = 35;
const EXECUTOR_LIMIT_POLICY:             usize = 36;
const EXECUTOR_DAILY_USAGE_WINDOW:       usize = 37;
...
const RENT_REFUND_RECIPIENT:             usize = 38;
...
const DISTINCT_ACCOUNT_INDEXES:       &[usize] = &[
    ...
    COUNTERPARTY_A_DAILY_USAGE_WINDOW,
    COUNTERPARTY_B_DAILY_USAGE_WINDOW,
...
const IDENTITY_ACCOUNT_INDEXES: &[usize] = &[EXECUTOR, PRINCIPAL_A, PRINCIPAL_B];
const PERMISSION_ACCOUNT_INDEXES: &[usize] = &[
    PRINCIPAL_A_PERMISSION_RECORD,
    PRINCIPAL_B_PERMISSION_RECORD,
    EXECUTOR_PERMISSION_RECORD,
];

#[derive(BorshDeserialize)]
pub struct MintTrilateralArgs {
    pub intent_id:  [u8; 32],
...
pub fn handle<'a>(accounts: &'a [AccountInfo<'a>], args_data: &[u8]) -> ProgramResult {
    ...
    let principal_a_permission_record_account_info = &accounts[PRINCIPAL_A_PERMISSION_RECORD];
    let principal_b_permission_record_account_info = &accounts[PRINCIPAL_B_PERMISSION_RECORD];
    ...
    let principal_b_account_info                   = &accounts[PRINCIPAL_B];
    ...
    assert_external_signer(principal_a_account_info, &program_id)?;
    assert_external_signer(principal_b_account_info, &program_id)?;
    ...
    let args = MintTrilateralArgs::try_from_slice(args_data)
    ...
    if !asset_pause_state_account_info.data_is_empty() {
        AssetPauseState::load_for_verified_pda(
            asset_pause_state_account_info,
            asset_mint_account_info.key,
            asset_pause_state_bump,
        )?.assert_not_paused_for(pause_bit::MINT, clock.slot)?;
    }
    ...
    pathway.assert_kind(pathway_kind::TRILATERAL)?;
    ...
    // swap to a different intent the principals didn't sign for.
    ...
    intent.assert_settlement_mode(settlement_mode::TRILATERAL_ATOMIC)?;
    ...
    if settlement_pol_id != [0u8; 32] {
        ...
        assert_settlement_policy_for_terms(
            ...
            asset_mint_account_info.key,
            principal_a_account_info.key,
            principal_b_account_info.key,
    ...
    // ── 4. Permissions - all three parties ───────────────────────────────────
    let _pa_rec = assert_subject_holds_role(
        principal_a_account_info.key,
    ...
    let _pb_rec = assert_subject_holds_role(
        principal_b_account_info.key,
    ...
    // Token-2022 collateral credits less than `asset_amount`. principal_a signs
    // the deposit; transfer before the rate, atomicity reverts it on any failure.
    ...
    let received = cpi_transfer_asset_in(
        asset_token_program_account_info, source_asset_token_account_info, asset_mint_account_info, reserve_asset_token_account_info, principal_a_account_info, asset_amount,
    )?;
    ...
    // Issued tokens minted to principal_b - not executor, not principal_a
    cpi_mint_to_with_decimals(
        issued_token_program_account_info, issued_token_mint_account_info, destination_issued_token_account_info, mint_authority_pda_account_info, bump, net_output_amount,
    ...
    if let Some((prepared_window_a, prepared_window_b)) = counterparty_daily_prepared {
        prepared_window_a.record_inflow(&accounts[COUNTERPARTY_A_DAILY_USAGE_WINDOW], asset_amount)?;
        prepared_window_b.record_inflow(&accounts[COUNTERPARTY_B_DAILY_USAGE_WINDOW], asset_amount)?;
    }
    ...
    // ── 11. Evidence - all three participants recorded ────────────────────────
    ...
    emit_settlement_mint(event_authority_account_info, event_authority_bump, SettlementMint {
        ...
        settlement_mode:        settlement_mode::TRILATERAL_ATOMIC,
        intent_id:              intent_id,
        ...
        principal_a:            *principal_a_account_info.key,
        principal_b:            *principal_b_account_info.key,
